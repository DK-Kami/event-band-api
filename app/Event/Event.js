...
class Event extends Model {
   ...
   */
  async getFormattingEvents(events) {
    return events;
  }
   ...
   * @param {Function} done колбэк-функция
   */
  async getFormattingEvent(event) {
    return event;
  }
}
 ...
 */
eventModel.getFilteredEvents = async (req, res) => {
  const {
 ...
 */
eventModel.getCurrentEvent = async (req, res) => {
  ...
  const event = await eventModel.getByUUID(eventUUID);
  if (!event) {
